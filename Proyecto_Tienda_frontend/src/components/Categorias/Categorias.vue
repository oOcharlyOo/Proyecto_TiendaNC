...
<style scoped>
.categorias-container{display:flex;flex-direction:column;gap:.6rem;padding:.6rem;height:100%;overflow:hidden}
.categorias-header{display:flex;align-items:center;justify-content:space-between;gap:.75rem;flex-wrap:wrap;flex-shrink:0}
.search-wrapper{position:relative;display:flex;align-items:center;flex:1;max-width:320px}.search-icon{position:absolute;left:.65rem;font-size:.8rem;pointer-events:none;opacity:.6}.search-input{width:100%;padding:.5rem 2rem .5rem 2rem;background:var(--color-bg-primary);border:none;border-radius:8px;color:var(--color-text-primary);font-size:.8rem;outline:none;box-shadow:inset 2px 2px 4px rgba(0,0,0,.1)}.search-input:focus{box-shadow:inset 2px 2px 4px rgba(0,0,0,.1),0 0 0 2px var(--color-accent)}.search-input::placeholder{color:var(--color-text-secondary);opacity:.5}.search-clear{position:absolute;right:.4rem;background:none;border:none;color:var(--color-text-secondary);cursor:pointer;padding:.2rem;font-size:.85rem;border-radius:50%;width:22px;height:22px;display:flex;align-items:center;justify-content:center}.search-clear:hover{background:var(--color-border);color:var(--color-text-primary)}
.btn-primary{border:none;padding:.55rem 1.1rem;font-size:.75rem;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--color-on-brand);background:var(--color-accent);cursor:pointer;box-shadow:4px 4px 10px rgba(0,0,0,.2),-1px -1px 4px rgba(255,255,255,.04);transition:all .2s;display:inline-flex;align-items:center;gap:.35rem;border-radius:8px;white-space:nowrap}.btn-primary:hover:not(:disabled){transform:translateY(-2px);box-shadow:6px 6px 16px rgba(0,0,0,.28),-3px -3px 8px rgba(255,255,255,.05)}.btn-primary:active:not(:disabled){transform:translateY(1px);box-shadow:inset 2px 2px 5px rgba(0,0,0,.2)}.btn-primary:disabled{opacity:.5;cursor:not-allowed;transform:none}.btn-icon{font-size:1rem}
.btn-secondary{border:none;padding:.5rem .85rem;font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--color-text-secondary);background:var(--color-bg-secondary);cursor:pointer;box-shadow:3px 3px 6px rgba(0,0,0,.1),-1px -1px 3px rgba(255,255,255,.02);transition:all .2s;display:inline-flex;align-items:center;gap:.3rem;border-radius:8px}.btn-secondary:hover{transform:translateY(-2px);box-shadow:5px 5px 12px rgba(0,0,0,.18);color:var(--color-text-primary)}.btn-secondary:active{transform:translateY(1px);box-shadow:inset 2px 2px 4px rgba(0,0,0,.15)}
.btn-danger{border:none;padding:.5rem .85rem;font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:#fff;background:linear-gradient(145deg,var(--color-error) 0%,color-mix(in srgb,var(--color-error) 60%,#000) 100%);cursor:pointer;box-shadow:4px 4px 10px rgba(0,0,0,.2);transition:all .2s;display:inline-flex;align-items:center;gap:.3rem;border-radius:8px}.btn-danger:hover{transform:translateY(-2px);box-shadow:6px 6px 16px rgba(0,0,0,.28)}.btn-danger:active{transform:translateY(1px);box-shadow:inset 2px 2px 5px rgba(0,0,0,.2)}
.categorias-table-container{flex:1;overflow:auto;min-height:0}.estado-loading,.estado-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1rem;padding:3rem 1rem;color:var(--color-text-secondary)}.loading-spinner{width:40px;height:40px;border:3px solid var(--color-border);border-top-color:var(--color-accent);border-radius:50%;animation:spinCat .8s linear infinite}@keyframes spinCat{to{transform:rotate(360deg)}}.empty-icon{font-size:2.5rem;opacity:.5}.empty-text{font-size:.9rem;font-weight:600}
.tabla-categorias{width:100%;border-collapse:collapse;font-size:.85rem}.tabla-categorias thead{position:sticky;top:0;z-index:5;background:var(--color-bg-panel)}.tabla-categorias th{padding:.6rem .8rem;text-align:left;font-weight:700;color:var(--color-accent);text-transform:uppercase;font-size:.65rem;letter-spacing:.06em;border-bottom:2px solid var(--color-border)}.text-center{text-align:center}.col-id{width:80px}.col-acciones{width:110px}
.categoria-row{border-bottom:1px solid rgba(255,255,255,.03);transition:all .1s}.categoria-row:hover{background:color-mix(in srgb,var(--color-accent) 5%,transparent)}.categoria-row td{padding:.55rem .8rem;vertical-align:middle}.id-badge{font-family:'Courier New',monospace;font-size:.75rem;color:var(--color-text-secondary);background:var(--color-bg-primary);padding:.15rem .45rem;border-radius:4px;box-shadow:1px 1px 2px rgba(0,0,0,.05)}.nombre-text{font-weight:700;color:var(--color-text-primary)}.desc-text{color:var(--color-text-secondary);font-size:.8rem}.acciones-cell{display:flex;align-items:center;justify-content:center;gap:.4rem}.btn-action{width:32px;height:32px;border:none;border-radius:6px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .15s;background:var(--color-bg-primary);box-shadow:2px 2px 4px rgba(0,0,0,.08)}.action-icon{width:16px;height:16px}.btn-edit{color:var(--color-accent)}.btn-edit:hover{background:var(--color-accent);color:var(--color-on-brand);transform:translateY(-1px);box-shadow:4px 4px 8px rgba(0,0,0,.15)}.btn-delete{color:var(--color-error)}.btn-delete:hover{background:var(--color-error);color:#fff;transform:translateY(-1px);box-shadow:4px 4px 8px rgba(0,0,0,.15)}
...
@media(max-width:600px){.categorias-header{flex-direction:column;align-items:stretch}.search-wrapper{max-width:none}.btn-primary{justify-content:center}.drawer{width:100%}}
</style>
